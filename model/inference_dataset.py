...
class CVInferenceDataset(Dataset):
    def __init__(self, meta_path, iterat, trainer_data, matrix_shape=(256,256), transform=None):
        if trainer_data:
            self.meta = read_csv(meta_path+'metadata.csv')[iterat:]
        else:
            self.meta = read_csv(meta_path+'metadata.csv')[:iterat]
    ...
    def __getitem__(self, idx):
        ...
        for i in range(0, image_res.shape[0], self.matrix_shape[0]):
            for j in range(0, image_res.shape[1], self.matrix_shape[1]):
                ...
                target_part = target_result[i:i+self.matrix_shape[0], j:j+self.matrix_shape[1]].astype(int64)
                data.append((image_part, target_part))
        return data
